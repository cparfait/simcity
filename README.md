...
- **Consommations** — une ligne par numéro, cumulée sur la période, avec recherche, filtres (forfait, service, signalement), tri par colonne et pagination. **Les totaux du pied de tableau portent sur toute la sélection filtrée, pas sur la page affichée**
  - *Clic sur un numéro* → l'historique complet de la ligne, sur toute sa profondeur facturée. Le graphique **grise les mois antérieurs à la prise en main par le titulaire actuel** et la carte d'en-tête donne la date de détention : la consommation d'avant est celle d'un titulaire précédent, et la lire comme celle du titulaire actuel est un contresens. Le tableau mensuel porte deux colonnes de nom côte à côte — *Titulaire (SimCity)* et *Nom sur la facture* — pour lire mois par mois ce que dit le référentiel et ce que dit l'opérateur ; l'écart est signalé en couleur, et un nom de facture qui change seul trahit un changement de titulaire jamais enregistré dans SimCity
...
**Qui est le titulaire : le référentiel fait foi.** Les consommations (liste et détail d'une ligne) portent **deux colonnes de nom côte à côte** — *Titulaire (SimCity)* et *Nom sur la facture* — et l'écart entre les deux est signalé en couleur. Partout ailleurs (top 10 du tableau de bord, alertes), c'est le **titulaire SimCity** qui est affiché, le nom de la facture ne servant qu'à défaut. C'est le référentiel qui décide des attributions, la facture ne fait que les refléter — souvent avec du retard, un nom de service ou un libellé de matériel. Seul l'onglet **Rapprochement des noms** compare les deux à parts égales, puisque c'est son objet.
